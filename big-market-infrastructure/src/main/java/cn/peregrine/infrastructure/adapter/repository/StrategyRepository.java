package cn.peregrine.infrastructure.adapter.repository;
...
import cn.peregrine.infrastructure.dao.po.*;
import cn.peregrine.infrastructure.redis.IRedisService;
...
import static cn.peregrine.types.enums.ResponseCode.UN_ASSEMBLED_STRATEGY_ARMORY;
...
//        if (strategyAwardEntities != null && !strategyAwardEntities.isEmpty()) {return strategyAwardEntities;}
    ...
    @Override
    public <K,V> void storeStrategyAwardSearchRateTable(String key, int rateRange, Map<K, V> strategyAwardSearchRateTable) {
        // 获取并更新存储抽奖策略范围值
    ...
    public Boolean subtractionAwardStock(String cacheKey, Date endDateTime) {
        ...
        // 按照cacheKey decr 后的值，如 99、98、97 和 key 组成为库存锁的key进行使用
        // 加锁为了兜底，如果后续有恢复库存，手动处理等，也不会超卖。因为所有的可用库存key，都被加锁了。
